...
- **that the alternative conventions are named rather than allowed as a band.** Truncating instead of rounding, and rounding half to even instead of half away from zero, are both real and neither is an error, so each is quiet and says which convention it assumed. A tolerance band was the first draft and it was wrong: one unit in the last place is a whole percentage point at zero decimals, which would have accepted 77 as a rendering of 76.3467
- **what it will not pair, which is most of the work.** Code is blanked before anything is read, so a figure in a fenced block cannot be married to one in the prose. A relation needs exactly one fraction and exactly one percentage in the sentence, no more than 80 characters apart. A sentence carrying two of either is ambiguous and reports nothing, which is the same rule the sources half already follows: a claim we cannot locate is a claim we must not touch. Each of those exists because the first draft produced four findings against this repository and every one was wrong
- **that a range means `between X and Y` only.** `from X to Y` is ordinary English for a decrease, and an earlier draft reported `reference/tighten.md` for saying a tighten took the variance from 0.61 to 0.34
- **that a file it cannot open is reported with the reason**, before anything else, and that if nothing could be read it says so and stops rather than explaining what it did not find in content it never opened
